    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">Issued {formatDate(prescription.created_at)}</p>
        </div>
        <Button variant="secondary" as={Link} to={PATIENT_ROUTES.PRESCRIPTIONS}>
          Back
        </Button>
      </div>
      <Card className="p-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <p className="mt-1 font-medium text-slate-900">{prescription.doctor_name || "-"}</p>
          </div>
          <div>
            ...
            <Card key={item.id} className="p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="text-right text-sm text-slate-700">
                  ...
                  {item.dispensed_quantity != null && <p className="text-slate-600">Dispensed: {item.dispensed_quantity}</p>}
                </div>
              </div>
            </Card>
          ))
